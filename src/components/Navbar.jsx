    ...
    <>
      <nav className="w-full bg-white h-16 z-30 relative">
        <div className="tablet:container tablet:px-8 mobile:px-4 desktop:px-0 h-full m-auto flex justify-between items-center">
          ...
          <button className="outline-none laptop:hidden" onClick={() => props.setShow(!props.show)}>{!!props.show ? <FiX /> : <FiMenu />}</button>
...
const Navbar = (props) => {
  ...
  return (
    <nav className="shadow-lg fixed w-full z-50">
      ...
      <motion.div className="mobile:px-4 tablet:px-0 mobile:bg-white mobile:w-full laptop:hidden z-10 absolute"
        animate={{ y: yLog }}
